...
It is used to gather entry metadata for all business objects: checks, bills.. -->
    ...
    <form action="{//runtime/link_prefix}check-submit&amp;entry_id={/_R_/_get/entry_id}" method="post" onSubmit="return validateStandard(this, 'myerror');">
      ...
      <div id="check">
        ...
        <input type="text" name="entry_datetime" style="width: 7em;" value="{substring($get_journal_entry/entry_date,0,11)}"/>
        </div>
        <div id="check_number">
          <xsl:value-of select="$i18n/check_number"/>:
        <input type="text" name="check_number" style="width: 5em;" value="{//get_some_business_objects/check_number}"/>
        </div>
        <div id="check_payee">
          <xsl:value-of select="$i18n/check_payee"/>:
        <input type="text" name="check_payee" value="{//get_some_business_objects/check_payee}"/>
        $<input type="text" name="entry_amount" style="width: 8em;" value="{$get_journal_entry/entry_amount}"/>
        </div>
        <div id="check_memo">
        ...
        <xsl:if test="count(/_R_/account_business_objects/account_id) &gt; 1">
          <select name="checking_account_id" required="1" exclude="-1" err="Please select a checking account.">
            ...
            <xsl:for-each select="//account_business_objects">
              ...
              <option value="{id}">
                <xsl:if test="id=//get_journal_entry/account_id and //get_journal_entry[account_id=$my_account_id]/entry_type_id='Debit'">
    ...
    -->
      <select name="expense_account_id" required="1" exclude="-1" err="Please select a credit account.">
        ...
        <xsl:for-each select="/_R_/get_all_accounts/get_all_accounts">
          ...
          <option value="{id}">
            <xsl:if test="id=//get_journal_entry/account_id and //get_journal_entry[account_id=$my_account_id]/entry_type_id='Credit'">
              <xsl:attribute name="selected">selected</xsl:attribute>
            </xsl:if>
            <xsl:value-of select="name"/>
          </option>
        ...
        <xsl:if test="not(//get_journal_entry/account_id=//get_all_accounts/get_all_accounts/account_id) and not(//get_journal_entry/status=9)">
